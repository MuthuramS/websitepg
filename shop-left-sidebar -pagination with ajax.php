...
<body>
    ...
    <!-- Property list will be dynamically populated here -->
</div>
                                    </div>
                                </div>
                            </div>
            ...
            <!-- Pagination links will be dynamically populated here -->
                    ...
                    <div class="col-lg-4  mb-100">
                        <aside class="sidebar ltn__shop-sidebar">
                            ...
                            <h4 class="ltn__widget-title">Zoning</h4>
                                <ul>
                                    <li>
                                        <label class="checkbox-item">House
                                            <input type="checkbox" name="house" checked="checked">
                                    ...
                                    <li>
                                        <label class="checkbox-item">Single Family
                                            <input type="checkbox" name="sigle family">
                                            <span class="checkmark"></span>
                                        </label>
                                ...
                                <!-- Possession Filter-->
                                <div class="widget--- ltn__price-filter-widget">
                                    <h4 class="ltn__widget-title ltn__widget-title-border---">Possession</h4>
                                    <div class="price_filter">
                                        <div class="price_slider_amount">
                                            <input type="submit" value="Start Date:" />
                                            <input type="date" class="amount" name="price" placeholder="Add Your Price" id="startDate" />
                                        </div>
                                        <div class="price_slider_amount">
                                            <input type="submit" value="End Date:" />
                                            <input type="date" class="amount" name="price" placeholder="Add Your Price" value="<?php echo date('Y-m-d'); ?>" id="endDate" />
                                        </div>
                                    </div>
                                </div>
                                <hr>
...
document.getElementById('endDate').valueAsDate = new Date();
    ...
    // Function to load more pages dynamically
    function loadMorePages() {
        var currentPage = <?php echo $currentPage; ?>;
        var totalPages = <?php echo $totalPages; ?>;
        var numPagesToShow = 10; // Number of pages to load dynamically

        // Calculate the start and end pages to load
        var startPage = currentPage + 1;
        var endPage = Math.min(totalPages, startPage + numPagesToShow - 1);

        // Append new pages to the pagination area
        var pagination = $('.ltn__pagination ul');
        for (var page = startPage; page <= endPage; page++) {
            var li = $('<li><a href="?<?php echo http_build_query($_GET); ?>&page=' + page + '">' + page + '</a></li>');
            pagination.append(li);
        }
    }

    // Example: Load more pages on button click
...
$(document).ready(function() {
    ...
    // Function to fetch properties based on page and type
    function fetchProperties(page, type) {
        $.ajax({
    ...
    // Function to display properties in HTML
    function displayProperties(properties) {
        var propertyListHtml = '';
        $.each(properties, function(index, property) {
            propertyListHtml += '<div class="col-xl-6 col-sm-6 col-12">';
            propertyListHtml += '<div class="ltn__product-item ltn__product-item-4 ltn__product-item-5 text-center">';
            propertyListHtml += '<a href="product-details.php?product_id=' + property.prpt_id + '" target="_blank">';
            propertyListHtml += '<div class="ltn__gallery-item-img">';
            propertyListHtml += '<img src="img/gallery/1.jpg" alt="' + property.elevation_image + '">';
            propertyListHtml += '<span class="pricing position-absolute">' + property.price_in_lacs_lower + ' - ' + property.price_in_lacs_upper + '</span>';
            propertyListHtml += '<div class="cover"></div>';
            propertyListHtml += '<span class="configration text-color-white mr-3 position-absolute">' + property.configuration.replace("BHK", "") + ' BHK</span>';
            propertyListHtml += '</div></a>';
            propertyListHtml += '<div class="ltn__gallery-item-info p-2">';
            propertyListHtml += '<h4><a href="portfolio-details.html">' + property.property_name + '</a></h4>';
            propertyListHtml += '<p><i class="icon-placeholder text-success ml-2"></i><span class="ml-2">' + property.location + ', Chennai</span></p>';
            propertyListHtml += '<div class="d-flex justify-content-between">';
            propertyListHtml += '<div><p><i class="fa fa-expand text-success ml-2"></i><span class="ml-2">' + property.min_size + ' - ' + property.max_size + ' SQ.FT</span></p></div>';
            propertyListHtml += '<div><p><i class="fa fa-calendar-alt text-success ml-2"></i><span class="ml-2">DEC 2024</span></p></div></div>';
            propertyListHtml += '<div class="d-flex"><ul class="d-flex list-inline list-group list-group-horizontal justify-content-center ml-auto mr-auto">';
            propertyListHtml += '<li><a href="#" title="Quick View" class="list-group-item"><i class="flaticon-expand"></i></a></li>';
            propertyListHtml += '<li><a href="#" title="Wishlist" class="list-group-item"><i class="flaticon-heart-1"></i></a></li>';
            propertyListHtml += '<li><a href="product-details.html" title="Product Details" class="list-group-item"><i class="flaticon-add"></i></a></li>';
            propertyListHtml += '</ul></div></div></div></div>';
        });
        $('#property-list').html(propertyListHtml);
    }

    // Function to update pagination links
    function updatePaginationLinks(currentPage, totalPages) {
        var paginationHtml = '';
        var numPagesToShow = 7; // Number of pages to show

        var startPage = Math.max(1, currentPage - Math.floor(numPagesToShow / 2));
        var endPage = Math.min(totalPages, startPage + numPagesToShow - 1);

        if (currentPage > 1) {
            paginationHtml += '<li><a href="#" data-page="1">&laquo;</a></li>';
        ...
        if (currentPage < totalPages) {
            ...
            paginationHtml += '<li><a href="#" data-page="' + totalPages + '">&raquo;</a></li>';
        }

        $('#pagination-links').html(paginationHtml);
    }

    // Initial fetch and display properties
    fetchProperties(currentPage, '<?php echo isset($_GET["type"]) ? $_GET["type"] : ""; ?>');

    // Pagination click event
    $(document).on('click', '.ltn__pagination ul li a', function(e) {
        e.preventDefault();
        var page = $(this).data('page');
        fetchProperties(page, '<?php echo isset($_GET["type"]) ? $_GET["type"] : ""; ?>');
    });
});
</script>
</body>
